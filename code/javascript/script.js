...
var preload = function() {
  ...
  this.load.spritesheet(Game.assetsKey, Game.assetsFilePath, {
    frameWidth: 64,
    frameHeight: 64,
    endFrame: 14
  });
...
class ChessPlayer {
  ...
  getDiagonalMoveOptions(gridPosition, boardPiece, boardToTest) {
    ...
    // ABOVE & RIGHT
    for (let row = gridPosition.row - 1, col = gridPosition.column + 1; row >= 0 && col < this.chessBoard.boardDimensions; row--, col++) {
      // Keep checking moves until one is invalid.
      move = new Move(gridPosition.row, gridPosition.column, row, col);
      results = this.checkMoveOptionValidityAndReturnMove(move, boardPiece.pieceColor, boardToTest);
      moves.push(...results[1]);
    ...
    // BELOW & LEFT
    for (let row = gridPosition.row + 1, col = gridPosition.column - 1; row < this.chessBoard.boardDimensions && col >= 0; row++, col--) {
      // Keep checking moves until one is invalid.
      move = new Move(gridPosition.row, gridPosition.column, row, col);
      results = this.checkMoveOptionValidityAndReturnMove(move, boardPiece.pieceColor, boardToTest);
      moves.push(...results[1]);
    ...
    // BELOW & RIGHT
    for (let row = gridPosition.row + 1, col = gridPosition.column + 1; row < this.chessBoard.boardDimensions && col < this.chessBoard.boardDimensions; row++, col++) {
      // Keep checking moves until one is invalid.
      move = new Move(gridPosition.row, gridPosition.column, row, col);
      results = this.checkMoveOptionValidityAndReturnMove(move, boardPiece.pieceColor, boardToTest);
      moves.push(...results[1]);
